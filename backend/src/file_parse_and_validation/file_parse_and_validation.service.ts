    ...
    if (extention == ".xlsx") {
      ...
      if (hasError) {
        ...
        fs.unlink(filePath, (err) => {
          if (err) {
            console.error(`Error cleaning up tempObsFiles`, err);
          } else {
            console.log(`Successfully cleaned up tempObsFiles.`);
          }
        })
      ...
      rowValidationStream
        ...
        .on("data", async (row) => {
          ...
          if (rowNumber == 2) {// TODO: remove this
              ...
              if (rowNumber == 2) {// TODO: remove this
                ...
                if (visitExists !== null && visitExists !== undefined) {
                  ...
                  try {
                    ...
                    this.logger.log(
                      "Visit record inserted in db successfully.",
                    );
                    fieldActivity["fieldVisit"] = visitInfo.fieldVisit;
                    fieldActivity["LocationID"] = rowData.LocationID;
                    GuidsToSave["visits"].push(visitInfo.fieldVisit);
                  ...
                  await this.prisma.aqi_imported_data.update({
                    where: {
                      aqi_imported_data_id:
                        guidsToUpdate[0].aqi_imported_data_id,
                    },
                    data: {
                      imported_guids: updatedJson,
                    },
