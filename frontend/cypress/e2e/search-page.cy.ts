...
describe('File Upload page functionality', () => {
  ...
  it('should conduct the search correctly', () => {
    cy.get('#search-button').should('exist').should('have.text', 'Search').click()

    cy.get('.MuiDataGrid-row').should('have.length', 10) //the default rows per page is 10
    cy.get('.MuiTablePagination-root .MuiTablePagination-select').should('be.visible').should('have.text', '10')
    cy.get('.MuiTablePagination-root .MuiTablePagination-displayedRows').should('be.visible').should('contain', '1–10')
    ...
    cy.get('.MuiTablePagination-root .MuiTablePagination-select').should('be.visible').should('have.text', '5')
    cy.get('.MuiTablePagination-root .MuiTablePagination-displayedRows').should('be.visible').should('contain', '1–5')
  })
})
